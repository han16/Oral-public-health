...
ggplot(data.frame(prop=round(unlist(y),3), code=rep(preventive_code, each=2), plan=rep(c("hmo", "ffs"), length(preventive_code))), aes(x=code, y=prop,fill=plan)) +geom_bar(position = "dodge", stat="identity")+
 # ylim(c(0, 0.7))+
  ylab("% unique ids")+
  #xlab("number of visits in the year")+
  theme(legend.position="bottom")+
...
ggplot(data.frame(prop=round(unlist(y),3), code=rep(restorative_code, each=2), plan=rep(c("hmo", "ffs"), length(restorative_code))), aes(x=code, y=prop,fill=plan)) +geom_bar(position = "dodge", stat="identity")+
 # ylim(c(0, 0.7))+
  ylab("% unique ids")+
  #xlab("number of visits in the year")+
  theme(legend.position="bottom")+
  theme(axis.text.x = element_text(angle = 30, vjust = 0.5, size=9))+
  geom_text(aes(label=prop*100), position=position_dodge(width=0.9), vjust=-0.25, size=3)+  # add numbers over bars
  ggtitle("Simple restorative")+
  theme(plot.title = element_text(hjust = 0.5, size=15))  #center the title

data.frame(prop=round(unlist(y),3), code=rep(restorative_code, each=2), plan=rep(c("hmo", "ffs"), length(restorative_code)))%>%
datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
